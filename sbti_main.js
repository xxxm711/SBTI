import { buildVisibleQuestions, getI18nValue, resolveLang } from './i18n_utils.mjs';
import {
    AUTHOR_TEXT,
    DIMENSION_META_I18N,
...
const TYPE_IMAGES = {
    ...
    "WOC!": "./assets/type-images/WOC_.png",
    "DRUNK": "./assets/type-images/DRUNK.png",
    "IMFW": "./assets/type-images/IMFW.png"
};
const TYPE_IMAGES_ES = {
    "IMSB": "./assets/images_es/IMSB_ES.png",
    "BOSS": "./assets/images_es/BOSS_ES.png",
    "MUM": "./assets/images_es/MUM_ES.png",
    "FAKE": "./assets/images_es/FAKE_ES.png",
    "Dior-s": "./assets/images_es/DIOR-s_ES.png",
    "DEAD": "./assets/images_es/DEAD_ES.png",
    "ZZZZ": "./assets/images_es/ZZZZ_ES.png",
    "GOGO": "./assets/images_es/GOGO_ES.png",
    "FUCK": "./assets/images_es/FUCK_ES.png",
    "CTRL": "./assets/images_es/CTRL_ES.png",
    "HHHH": "./assets/images_es/HHHH_ES.png",
    "SEXY": "./assets/images_es/SEXY_ES.png",
    "OJBK": "./assets/images_es/OJBK_ES.png",
    "JOKE-R": "./assets/images_es/JOKER-R_ES.png",
    "POOR": "./assets/images_es/POOR_ES.png",
    "OH-NO": "./assets/images_es/OH-NO_ES.png",
    "MONK": "./assets/images_es/MONK_ES.png",
    "SHIT": "./assets/images_es/SHIT_ES.png",
    "THAN-K": "./assets/images_es/THAK-K_ES.png",
    "MALO": "./assets/images_es/MALO_ES.png",
    "ATM-er": "./assets/images_es/ATM-er_ES.png",
    "THIN-K": "./assets/images_es/THIN-K_ES.png",
    "SOLO": "./assets/images_es/SOLO_ES.png",
    "LOVE-R": "./assets/images_es/LOVE-R_ES.png",
    "WOC!": "./assets/images_es/WOC_ES.png",
    "DRUNK": "./assets/images_es/DRUNK_ES.png",
    "IMFW": "./assets/images_es/IMFW_ES.png"
};
const TYPE_IMAGES_BY_LANG = {
    "zh-CN": TYPE_IMAGES,
    en: TYPE_IMAGES_ES,
    es: TYPE_IMAGES_ES,
};
...
const LANGUAGE_OPTIONS = ['zh-CN', 'en', 'es'];

function getQuestionsForLang(lang) {
    return QUESTION_SETS[lang] || QUESTION_SETS['zh-CN'];
}

function getSpecialQuestionsForLang(lang) {
    return SPECIAL_QUESTION_SETS[lang] || SPECIAL_QUESTION_SETS['zh-CN'];
}

function getTypeLibraryForLang(lang) {
    return TYPE_LIBRARY_I18N[lang] || TYPE_LIBRARY_I18N['zh-CN'];
}

function getDimensionMetaForLang(lang) {
    return DIMENSION_META_I18N[lang] || DIMENSION_META_I18N['zh-CN'];
}

function getDimExplanationsForLang(lang) {
    return DIM_EXPLANATIONS_I18N[lang] || DIM_EXPLANATIONS_I18N['zh-CN'];
}

function getTypeImagesForLang(lang) {
    return TYPE_IMAGES_BY_LANG[lang] || TYPE_IMAGES;
}

function t(key) {
    return getI18nValue(UI_TEXT, app.lang, [key]);
}
...
function renderStaticTexts() {
    ...
    document.getElementById('backIntroBtn').textContent = t('backHomeBtn');
    document.getElementById('submitBtn').textContent = t('submitBtn');
    document.getElementById('restartBtn').textContent = t('restartBtn');
    document.getElementById('toTopBtn').textContent = t('toTopBtn');

    const analysisTitle = document.querySelector('.analysis-box h3');
    if (analysisTitle) analysisTitle.textContent = t('analysisTitle');

    const dimTitle = document.querySelector('.dim-box h3');
    if (dimTitle) dimTitle.textContent = t('dimensionTitle');

    const noteTitle = document.querySelector('.note-box h3');
    if (noteTitle) noteTitle.textContent = t('noteTitle');

    const author = AUTHOR_TEXT[app.lang] || AUTHOR_TEXT['zh-CN'];
    const authorSummary = document.querySelector('.author-box summary');
    const authorPs = document.querySelectorAll('.author-content p');
    if (authorSummary) authorSummary.textContent = author.summary;
    if (authorPs[0]) authorPs[0].textContent = author.p1;
    if (authorPs[1]) authorPs[1].textContent = author.p2;
    if (authorPs[2]) authorPs[2].textContent = author.p3;
    if (authorPs[3]) authorPs[3].textContent = author.p4;
    if (authorSummary) {
        authorSummary.dataset.expandText = t('authorExpand');
        authorSummary.dataset.collapseText = t('authorCollapse');
    }
}
...
function setLanguage(lang) {
    ...
    if (!LANGUAGE_OPTIONS.includes(next)) return;
    if (next === app.lang) return;
    app.lang = next;
    remapShuffledQuestionsForLanguage();
    rerenderCurrentScreen();
}

function remapShuffledQuestionsForLanguage() {
    if (!Array.isArray(app.shuffledQuestions) || app.shuffledQuestions.length === 0) return;

    const regularMap = Object.fromEntries(
        getQuestionsForLang(app.lang).map((q) => [q.id, q]),
    );
    const specialMap = Object.fromEntries(
        getSpecialQuestionsForLang(app.lang).map((q) => [q.id, q]),
    );

    app.shuffledQuestions = app.shuffledQuestions.map((q) => {
        if (q.special) return specialMap[q.id] || q;
        return regularMap[q.id] || q;
    });
}
...
function getVisibleQuestions() {
    return buildVisibleQuestions(app.shuffledQuestions, getSpecialQuestionsForLang(app.lang), app.answers);
}
...
function computeResult() {
    const dimensionMeta = getDimensionMetaForLang(app.lang);
    const questions = getQuestionsForLang(app.lang);
    const TYPE_LIBRARY = getTypeLibraryForLang(app.lang);
    const rawScores = {};
    const levels = {};
    ...
    let finalType;
    let modeKicker = t('modeYourPrimary');
    let badge = tf('badgeNormal', { similarity: bestNormal.similarity, exact: bestNormal.exact });
    let sub = t('subNormal');
    let special = false;
    ...
    if (drunkTriggered) {
        ...
        modeKicker = t('modeHiddenActivated');
        badge = t('badgeDrunk');
        sub = t('subDrunk');
        special = true;
    } else if (bestNormal.similarity < 60) {
        finalType = TYPE_LIBRARY.HHHH;
        modeKicker = t('modeFallback');
        badge = tf('badgeFallback', { similarity: bestNormal.similarity });
        sub = t('subFallback');
        special = true;
    } else {
...
function renderDimList(result) {
    const dimensionMeta = getDimensionMetaForLang(app.lang);
    const DIM_EXPLANATIONS = getDimExplanationsForLang(app.lang);
...
function renderResult() {
    ...
    const typeImages = getTypeImagesForLang(app.lang);
...
function startTest(preview = false) {
    const questions = getQuestionsForLang(app.lang);
    const specialQuestions = getSpecialQuestionsForLang(app.lang);
